...
class MuSL(object,metaclass=MuSLType):
    ...
    #=====================================================================
    """
    Hacemos que 'data' y 'sl' sean read only definiendo sólo el getter usando el decorador @property.
    """
    @property
    def data(self):
        return self._data
    ...
    #=====================================================================
    """
    Definimos una serie de rutinas para poder crear clases de tipo MuRecon desde distintos objetos.
    """
    @classmethod
    def from_MuData(cls,mu_data,sl,slstudy_cfg='3n4hits'):
        """
        Función para crear objetos tipo MuRecon a partir de los datos de una superlayer
        """
        data_for_recon = mu_data.copy()
        data_for_recon.add_filter(load_cfg(SLStudy,config_name='_'.join([f'0{sl}',slstudy_cfg])))
        return cls(data_for_recon,sl)
    
    @classmethod
    def from_run(cls,run,sl,slstudy_cfg='3n4hits'):
        """
        Función para crear objetos tipo MuRecon a partir de los datos de una superlayer
        """
        data_for_recon = MuData(run).clean()
        data_for_recon.add_filter(load_cfg(SLStudy,config_name='_'.join([f'0{sl}',slstudy_cfg])))
        return cls(data_for_recon,sl)
    ...
    def _set_pattern(self):
        ...
        self._pattern = self.pattern_from_df(df)

    @classmethod
    def pattern_from_df(cls,df):
        patt = pd.DataFrame({})
        for i in layers:
        ...
        patt = patt.apply(cls.calc_pattern,axis=1)
        return patt

    @staticmethod
    def calc_pattern(ser):
